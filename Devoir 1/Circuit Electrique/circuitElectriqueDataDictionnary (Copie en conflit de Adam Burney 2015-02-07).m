...
%Représentation d'état linéarisée autour du point d'équilibre:
...
for i=1:length(Ais)
    ...
    wSaveFileName        = strcat('circuit_Ai_',strrep(num2str(Ai),'.','dec'));
    wLinearOutputName    = strcat('linear_',wSaveFileName);
    wNonLinearOutputName = strcat('nonLinear_',wSaveFileName);
    
    set_param(model,'StopFcn','save(wSaveFileName,wLinearOutputName,wNonLinearOutputName)');
    set_param(strcat(model,'/linearOutput'),'VariableName',wLinearOutputName);
    set_param(strcat(model,'/nonLinearOutput'),'VariableName',wNonLinearOutputName);    
